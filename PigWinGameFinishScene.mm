...
@implementation PigWinGameFinishScene

+ (id)scene
{
    CCScene* sc = [CCScene node];
    PigWinGameFinishScene* finishSC = [[PigWinGameFinishScene alloc] init];
...
{
    CCScene* sc = [CCScene node];
    PigWinGameFinishScene* gs = [PigWinGameFinishScene nodeWithLevel:level];
    [sc addChild:gs];
    //[gs release];
    return sc;
}
...
- (id)initWithLevel:(int)level
{
    ...
    if (self) {
        ...
        CCSprite *sp = [CCSprite spriteWithFile:@"pigsbg.png"];
        [sp setScale:0.5f];
...
-(void) winLoseLabel{
    //添加you win / you lose 动态效果
    CCLabelTTF* label = [CCLabelTTF labelWithString:@"Congratulations!" fontName:@"Schwarzwald Regular" fontSize:25];
    id actionScale = [CCScaleTo actionWithDuration:0.0f scale:0.4f];
    ...
    //nextLevel 按钮
    CCSprite* nextLevelSprite = [CCSprite spriteWithFile:@"next.png"];
    [nextLevelSprite setScale:0.5f];
    CCMenuItemSprite* nextLvMenuItem = [CCMenuItemSprite itemFromNormalSprite:nextLevelSprite selectedSprite:nil target:self selector:@selector(nextLevel:)];
    
    CCSprite* menu =  [CCMenu menuWithItems:replayMenuItem, nil];
    CCSprite* menu2 =  [CCMenu menuWithItems:selectLvMenuItem, nil];
    CCSprite* menu3 = [CCMenu menuWithItems:nextLvMenuItem, nil];
    [menu setPosition:ccp(140.0f,60.0f)];
    [menu2 setPosition:ccp(260.0f,60.0f)];
    [menu3 setPosition:ccp(380.0f, 60.0f)];
    [self addChild:menu];
    [self addChild:menu2];
    [self addChild:menu3];
...
{
    //小鸟动作
    CCSprite* bird = [CCSprite spriteWithFile:@"pig_11.png"];
    ...
    //小猪动作
    NSString* pigName = [NSString stringWithFormat:@"pig1.png"];
...
-(void)nextLevel:(id)arg
{
    NSLog(@"next level");
    if (currLevel + 1 > 14) {
        CCScene* nextScene = [CongratulationScene scene];
        CCTransitionScene* trans = [[CCTransitionPageTurn alloc] initWithDuration:0.5f scene:nextScene];
        [[CCDirector sharedDirector] replaceScene:trans];
        [trans release];
    }
    else {
        //printf("currLV is : %d\n", currLevel);
        CCScene* nextScene = [GameScene sceneWithLevel:currLevel + 1];
        CCTransitionScene* trans = [[CCTransitionPageTurn alloc] initWithDuration:0.5f scene:nextScene];
        [[CCDirector sharedDirector] replaceScene:trans];
        [trans release];
    }
